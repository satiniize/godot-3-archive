...
var current_movement = movement.WALKING

func float_interpolate(_start: float, _end: float, _bias: float) -> float:
	return _start * (1.0 - _bias) + _end * _bias

func handle_accel(accel: float, max_speed: float, on_floor: bool, _delta: float) -> void:
	accel_dir *= Vector3(1.0, 0.0, 1.0)
	var final_vel = velocity + accel_dir * accel * _delta
	if (final_vel.dot(accel_dir) < max_speed):
		velocity = final_vel

func limit_speed(max_speed):
...
func handle_friction(friction: float, delta: float) -> void:
...
func handle_movement(delta: float):
	handle_input()
	velocity = move_and_slide(velocity, Vector3.UP, true, 4, PI/2.0)
	handle_drag(delta)
	match current_movement:
		movement.WALKING:
			$CollisionShapeUpper.translation.y = float_interpolate($CollisionShapeUpper.translation.y, 2, delta * 8.0)
			$CollisionShapeUpper/CamGroup/Camera.fov = float_interpolate($CollisionShapeUpper/CamGroup/Camera.fov, 90, delta * 8.0)
			velocity.y -= gravity * delta
			if is_on_floor():
				handle_accel(walk_force / mass, max_speed_walk, is_on_floor(), delta)
				handle_friction(0.1, delta)
			if Input.is_action_just_pressed("move_run"):
				current_movement = movement.RUNNING
			if Input.is_action_just_pressed("move_crouch"):
				current_movement = movement.CROUCHING
				
		movement.RUNNING:
			$CollisionShapeUpper.translation.y = float_interpolate($CollisionShapeUpper.translation.y, 2, delta * 8.0)
			$CollisionShapeUpper/CamGroup/Camera.fov = float_interpolate($CollisionShapeUpper/CamGroup/Camera.fov, 100, delta * 8.0)
			velocity.y -= gravity * delta
			if is_on_floor():
				handle_accel(walk_force / mass, max_speed_walk, is_on_floor(), delta)
				handle_friction(0.1, delta)
			...
			if Input.is_action_just_pressed("move_crouch"):
				current_movement = movement.SLIDING
				
		movement.AIRSTRAFING:
			pass
			
		movement.CROUCHING:
			$CollisionShapeUpper.translation.y = float_interpolate($CollisionShapeUpper.translation.y, 1, delta * 8.0)
			$CollisionShapeUpper/CamGroup/Camera.fov = float_interpolate($CollisionShapeUpper/CamGroup/Camera.fov, 80, delta * 8.0)
			velocity.y -= gravity * delta

			if is_on_floor():
				handle_accel(walk_force / mass, max_speed_crouch, is_on_floor(), delta)
				handle_friction(0.1, delta)
				limit_speed(max_speed_crouch)
			if velocity.length() > max_speed_crouch * 1.1:
				current_movement = movement.SLIDING
		...
		movement.SLIDING:
			$CollisionShapeUpper.translation.y = float_interpolate($CollisionShapeUpper.translation.y, 1, delta * 8.0)
			$CollisionShapeUpper/CamGroup/Camera.fov = float_interpolate($CollisionShapeUpper/CamGroup/Camera.fov, 110, delta * 8.0)
			velocity.y -= gravity * delta
			if is_on_floor():
				handle_accel(walk_force / 20.0 / mass, 100.0, is_on_floor(), delta)
				handle_friction(0.8, delta)
		...
		movement.WALLRUNNING:
			pass
#			handle_wall_run(is_on_wall(), walk_dir, delta)
...
#	velocity = move_and_slide_with_snap(velocity, -get_floor_normal(), Vector3.UP, true, 4, PI/2.0)
#			handle_crouch(delta)
